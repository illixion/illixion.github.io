...
- **Trust anchor** — [pinned_signers](updater/pinned_signers) is `go:embed`-ed into every binary ([pin.go](updater/pin.go)). It holds two ed25519 public keys (a daily YubiKey signer + an offline backup). The deployment URL is **not** compiled in.
- **Location resolution** ([resolve.go](updater/resolve.go), [discovery.go](updater/discovery.go), [location.go](updater/location.go), [prompt.go](updater/prompt.go)) — a domain arg → fetch `<domain>/discovery.json` (untrusted; location/cadence only) → save resolved location to `.ssh-keys-updater.conf` next to `authorized_keys`. This is why a host move (Cloudflare → GitHub Pages) needs no rebuild. A forged discovery.json can at worst cause a missed update, never a key injection.
- **Signature verification** ([verify.go](updater/verify.go), [sshwire.go](updater/sshwire.go)) — SSHSIG is verified **natively in Go**, reducing the trust path to one `crypto/ed25519.Verify`. This is deliberate: Windows and OpenWRT/dropbear lack `ssh-keygen -Y verify`. The manifest JSON is parsed only *after* the signature verifies.
- **Update cycle** ([update.go](updater/update.go), [manifest.go](updater/manifest.go), [state.go](updater/state.go)) — fetch → verify SSHSIG against a pinned, non-revoked signer → check `serial` strictly increases → record any `disable_signer` revocation → atomically write (temp+rename, mode 0600) the managed block followed by `authorized_keys_local` verbatim → persist state. **On any failure the existing `authorized_keys` is left untouched** — a hostile manifest can fail to add a key but can never remove access.
- **State** (`~/.ssh/.ssh-keys-updater.state`) — highest accepted `serial` + `disabled` signer fingerprints (honored forever, independent of serial).
- **Platform layers** — schedulers ([schedule.go](updater/schedule.go) + `schedule_{darwin,linux,windows}.go`: launchd / systemd-or-cron / schtasks) and paths ([paths_{linux,windows,other}.go](updater/), [secure_{unix,windows}.go](updater/), [fsutil.go](updater/fsutil.go)). Build tags select per-OS files; keep new OS-specific code behind the same tag pattern. Windows admin installs write `administrators_authorized_keys` and must reset its ACL via `icacls` or sshd silently refuses the file.

CLI subcommands (`main.go`): `run`, `install`, `uninstall`, `verify`, `gen-page`, `print-pins`, `version`. Flags and the optional `[domain]` positional may appear in any order.
